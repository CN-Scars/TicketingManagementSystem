...
![门票E-R图](E-R_Ticket_Management.png)
...
购买：（订单编号，用户编号，营业员编号，门票编号，购买数量，总价格，购买时间）；
...
### 4.1 与 E-R 图对应的关系模式
#### 实体所对应的关系模式：
1. 用户（<u>用户编号</u>，用户名，邮箱，年龄）
2. 门票（<u>门票ID</u>，价格，库存，门票名）
3. 营业员（<u>营业员编号</u>，姓名）

#### 联系所对应的关系模式：
1)  购买（<u>订单编号</u>，购买数量，总价，时间，状态，用户名，用户编号，门票编号，门票名，营业员编号，姓名）
### 4.2 优化后的数据模型
1. 用户（<u>用户名</u>，邮箱，用户编号，年龄）3NF
2. 营业员（营业员编号，姓名）3NF
3. 门票（<u>门票编号</u>，价格，门票余量，门票名）3NF
4. 购买（<u>营业员编号，用户编号，门票编号</u>，购买数量，总价，时间，状态）（营业员编号，用户编号和门票编号是外键）3NF

## 五、物理结构设计

由于MySQL会在有主键的情况下自动创建索引，故不再需要手动创建。
...
存放完整项目仓库的地址为：https://github.com/CN-Scars/TicketingManagementSystem
